...
data_fence = np.load("fence.npy")

# %%
# visualize your 5th category of images
show_image(data_fence)
...
X = np.vstack((data_cactus, data_camel, data_circle, data_door, data_fence))
...
y = np.concatenate((
    np.full(len(data_cactus), 0),
    np.full(len(data_camel), 1),
    np.full(len(data_circle), 2),
    np.full(len(data_door),3 ),
    np.full(len(data_fence), 4)
))
